...
$(document).ready(function(e) {
    ...
    <select class="select1" name="c_gid">
        <?php if(is_array($gra)): $i = 0; $__LIST__ = $gra;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$gr): $mod = ($i % 2 );++$i; $a = $gr["g_id"]; ?>
         <?php if($as["c_gid"] == $a): ?><option value="<?php echo ($gr["g_id"]); ?>" selected><?php echo ($gr["g_name"]); ?>级</option>
             <?php else: ?> <option value="<?php echo ($gr["g_id"]); ?>"><?php echo ($gr["g_name"]); ?>级</option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
    </select>
    </div>  
    </li>  
    <li><label>班级名称</label>
		<input name="c_name"  type="text" class="dfinput" value="<?php echo ($as["c_name"]); ?>" /><i>例：编程*班/美工*班</i>
	</li>
        <input type="hidden" name="c_id" value="<?php echo ($as["c_id"]); ?>" />
    <li><label>&nbsp;</label><?php if($as["c_id"] == ''): ?><input name="sub" type="submit" class="btn" value="添 加"/><?php else: ?><input name="sub" type="submit" class="btn" value="修 改"/><?php endif; ?></li>
    </ul>
    </form>
