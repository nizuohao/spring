...
function checkYes(id,xh){	$.get("/Spring/index.php/Admin/StuInfo/checkYes",{id:id},	function(data)	{		if(data == 1)	       {	    	   $(".sh").eq(xh-1).replaceWith("<span class='sh'>已通过</span>");		   }			}	)	return false;}function checkNo(id,xh){	$.get("/Spring/index.php/Admin/StuInfo/checkNo",{id:id},	function(data)	{		if(data == 1)	       {	    	   $(".sh").eq(xh-1).replaceWith("<span class='sh'>不通过</span>");		   }			}	)	return false;}
</script>
	<div class="place">
    <span>位置：</span>
    <ul class="placeul">
    ...
    <div class="tools">
    	...
    	<ul class="seachform">
    <form action="/Spring/index.php/Admin/StuInfo/stuInfoSel" method="post">      
    ...
    <table class="tablelist">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$si): $mod = ($i % 2 );++$i;?><tr class="click2" style="border-bottom:1px solid #ccc; border-top:1px solid #ccc;">
            ...
            <td><?php if($si["si_state"] == 0): ?><span class='sh'>未通过</span>	    <?php elseif($si["si_state"] == 1): ?>	    <span class='sh'>已通过</span>	    <?php else: ?>	    <span class="sh">		    <a href="" onclick="return checkYes(<?php echo ($si["si_id"]); ?>,<?php echo ($key-1); ?>);">通过</a> /		    <a href="" onclick= "return checkNo(<?php echo ($si["si_id"]); ?>,<?php echo ($key-1); ?>)">不通过</a>	    </span><?php endif; ?></td>
            <td><a href="/Spring/index.php/Admin/StuInfo/stuInfoSle?id=<?php echo ($si["si_id"]); ?>">查看评论</a></td>
            <td><a href="/Spring/index.php/Admin/StuInfo/stuInfoDel?id=<?php echo ($si["si_id"]); ?>" onClick="return confirm('您确定删除吗?')">删除</a></td>
