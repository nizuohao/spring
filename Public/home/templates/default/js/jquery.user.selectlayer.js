...
function OpenCategoryLayer(click_obj,showid,input,input_cn,input_txt,QSarr,strlen)
{
	...
	{
			$(this).blur();
			$(this).parent(div).before(div class=menu_bg_layerdiv);
			$(.menu_bg_layer).height($(document).height());
			$(.menu_bg_layer).css({ width $(document).width(), position absolute,left0, top0,z-index0,background-color#000000});
			$(.menu_bg_layer).css(opacity,0);
			$(showid+ .OpenFloatBoxBg).css(opacity, 0.2);
			$(showid).show();			
			$(showid+ .OpenFloatBox).css({left($(document).width()-$(showid+ .OpenFloatBox).width())2,top120});
			SetBoxBg(showid);
			$(showid+ .item).unbind(hover).hover(
				...
				{
					如果有子类则不能选择顶级分类
					$(this).find(.titem checkbox).attr({'checked'false,disableddisabled});
					...
					$(showid+ .OpenFloatBox label).unbind().click(function()
					{
						...
						{
							if ($(this).parent().find(checkbox[checked]).length0)
							{
			...
			function SetInput(showid,input_cn,input,strlen)
			{
					...
					var a_id=new Array();
					var i=0;
					if ($(showid+ .OpenFloatBox .selecteditem checkbox[checked]).length8)
					{
						alert(不能超过8个选项);
						return false;
					}
...
function SetBoxBg(showid)
{
				...
				$(showid+ .OpenFloatBoxBg).css({displayblock,width(FloatBoxWidth+12)+px,height(FloatBoxHeight+12)+px});
...
function MakeLi(val,pid)
{
			...
			var htmlstr='';
				...
				{
				 var v=arr[x].split(,);
				htmlstr+=labelinput type=checkbox value=+v[0]+ title=+v[1]+ class=s id=+pid++v[1]+labelbr;
				}
			return htmlstr; 
}
拷贝
...
{
					var htmlstr='&nbsp;&nbsp;&nbsp;已经选择分类：span class=empty[清空已选]spanbr';
					$(showid+ .item checkbox[checked][class='b']).each(function(index){
					htmlstr+=labelinput class=b  type=checkbox value=+$(this).attr(value)+ title=+$(this).attr(title)+ checked+$(this).attr(title)+label;
					})
					$(showid+ .item checkbox[checked][class='s']).each(function(index){
					 if ($(this).parent().parent().prev().find(checkbox).attr('checked')==false)
					 {						 
					htmlstr+=labelinput class=s  type=checkbox id=+$(this).attr(id)+ value=+$(this).attr(value)+ title=+$(this).attr(title)+ checked+$(this).attr(title)+label;
					 }
					})
					htmlstr+=div class=cleardiv;
					$(showid+ .selecteditem).html(htmlstr);
					if ($(showid+ .item checkbox[checked]).length0)
					{
...
function showmenu(menuID,showID,inputname,Form,Forminput)
{
	$(menuID).click(function(){
		...
		生成背景
		$(menuID).parent(div).before(div class=menu_bg_layerdiv);
		$(.menu_bg_layer).height($(document).height());
...
function OpentradeLayer(click_obj,input,input_cn,input_txt,showid,strlen)
{
	$(click_obj).click(function()
	{
			...
			$(showid+ .OpenFloatBoxBg).css(opacity, 0.2);
			$(showid).show();			
			$(showid+ .OpenFloatBox).css({left($(document).width()-$(showid+ .OpenFloatBox).width())2,top150});
			SetBoxBg(showid);
			$(showid+  label).hover(function()	{$(this).css(background-color,#E3F0FF);},function(){	$(this).css(background-color,);});
				$(showid+  label).unbind().click(function(){
						if ($(showid+ .content checkbox[checked]).length5)
						{
							alert(不能超过5个选项);
							$(this).attr(checked,false);
							return false;
...
function showtaglayer(menuID,showID,inputname,inputname1,strlen,get)
{
	...
	$(menuID).click(function(){
		...
		$(showID+  label).unbind().click(function(){
						if ($(showID+ checkbox[checked]).length5)
						{
							alert(不能超过5个选项);
							$(this).attr(checked,false);
							return false;
		...
		$(showID+ulli).click(function(){
			...
			if (strclass)假如存在小类
			{
				...
				$(showID+_s).html(go_back+getcathtml(strclass));生成LI
					...
					$(showID+_sulli).click(function(){点击小类	
...
function showyearbox(inputname)
{
	$(inputname).click(function(){
		var input=$(this);
		$(input).parent(div).before(div class=menu_bg_layerdiv);
		$(.menu_bg_layer).height($(document).height());
		$(.menu_bg_layer).css({ width $(document).width(), position absolute, left 0, top 0 , z-index 0});
		...
		htm+=div class=tit请选择年份div;
		htm+=ul;
		for (i=y;i=ymin;i--)
		{
		htm+=li title=+i+年+i+年li;
		}
		htm+=div class=cleardiv;
		htm+=ul;
		htm+=div;
		
		htm+=div class=showyearbox monthlist;
		htm+=div class=tit请选择月份div;
		htm+=ul;
		for (i=1;i=12;i++)
		{
		htm+=li title=+i+月+i+月li;
		}
		htm+=div class=cleardiv;
		htm+=ul;
		htm+=div;
